...
const privateKey = "afd66483d762b8a3596f9716d23c762afb860e83a2b2d09309baef304c8208bb";
const keyPair = ec.keyFromPrivate(privateKey, "hex");
const publicKey = keyPair.getPublic("hex");
// 04bf617dc6d6e0583f2129954c4e0210eb2854751d9c78d466f8b04446ef6978cce6be47c4569c9d350520492d620a8474afe7f89153d7d898dee78e56bd71f4f8

const WS = require("ws");

const PORT = process.env.PORT || 3005;
const MY_ADDRESS = process.env.MY_ADDRESS || "ws://localhost:3005";
const PEERS = process.env.PEERS ? process.env.PEERS.split(","): ["ws://localhost:3003", "ws://localhost:3004"];
    ...
    socket.on("message", message => {
        ...
        switch(_message.type) {
            ...
            case "TYPE_REPLACE_CHAIN":
                ...
                const theirTx = [...newBlock.data.filter(transaction => transaction.from !== MINT_PUBLIC_ADDRESS).map(transaction => JSON.stringify(transaction))];
                ...
                if (newBlock.prevHash !== DatChain.getLastBlock().prevHash) {
                    ...
                    // Yêu cầu các node khác gửi về block cho node hiện tại
                    ...
                    setTimeout(() => {
                        ...
                        check.splice(0, check.length); // Xóa toàn bộ phần tử trong check
                    }, 5000)
                }
                ...
                case "TYPE_REQUEST_CHECK":
                    ...
                    //      "data": Địa chỉ
                    // }
                    console.log("Data " , _message.data, "TYPE ", typeof(_message.data))
                    opened.forEach(node => console.log(node))
                ...
                case "TYPE_REQUEST_CHAIN":    // Dùng để yêu cầu chain
...
// Node 3 không có tiền nên không thể tạo transaction
// setTimeout( () => {
//     const transaction = new Transaction(publicKey, "04bf4187b0854e2317cbfd31b9a3c2503448284f154be98db61479c07af46e7a33be8504c0cc1b58b1ca2f2195a3a36e4184b93af00381dd74b7446fba85cea77e", 430, 10)
...
setTimeout(() => {
//     // Request để đồng bộ chain
    console.log(opened)
    // sendMessage(produceMessage("TYPE_REPLACE_CHAIN", "ws://localhost:3004"));
    console.log(DatChain)
}, 15000)

setTimeout(() => {
    if(DatChain.transactions.length !== 0) {
        DatChain.mineTransaction(publicKey);

        sendMessage(produceMessage("TYPE_REPLACE_CHAIN", [
            DatChain.getLastBlock(),
            DatChain.difficulty
        ]))
    }
}, 35000)
